...
export class UsuarioModel {
    ...
    static async getAll() {
        const sql = "SELECT `Usuario`, `Estado`, `Rol` FROM `usuarios` ORDER BY `UsuarioId` DESC";
    ...
    static async insert(usuarioData) {
        const sql = "INSERT INTO `usuarios`(`Usuario`, `Password`, `Estado`, `Rol`) VALUES (?, ?, ?, ?)";
        const [rows, fields] = await pool.execute(sql, usuarioData);
        return rows;
    }

    static async update(usuarioData) {
        const sql = "UPDATE `usuarios` SET `Usuario` = ?, `Password` = ?, `Estado` = ?, `Rol` = ? WHERE `UsuarioId` = ?";
        const [rows, fields] = await pool.execute(sql, usuarioData);
        return rows;
    }
